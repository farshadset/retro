...
test.describe('Home actions and profile registration', () => {
  test('profile supports register, friends requests, and online game', async ({ page, context }) => {
    const seed = Date.now()
    ...
    await expect(secondPage.getByTestId('home-banner-message')).toContainText('ثبت نام با موفقیت انجام شد')
    ...
    await expect(page.getByTestId('home-banner-message')).toContainText(`درخواست دوستی برای ${userB} ارسال شد.`)
    await expect(page.getByText(userB, { exact: true }).first()).toBeVisible()

    await expect(secondPage.getByTestId('friends-incoming-count')).toContainText('1 درخواست جدید', { timeout: 10000 })
    await expect(secondPage.getByTestId(`friends-incoming-row-${userA}`)).toContainText(
      `${userA} درخواست دوستی ارسال کرده است.`
    )
    await expect(secondPage.getByTestId(`friends-accept-${userA}`)).toBeVisible()
    await expect(secondPage.getByTestId(`friends-reject-${userA}`)).toBeVisible()
    await secondPage.getByTestId(`friends-accept-${userA}`).click()
    await expect(secondPage.getByTestId('home-banner-message')).toContainText(`درخواست ${userA} تایید شد.`)
    await expect(secondPage.getByTestId('friends-list')).toContainText(userA)

    await page.reload()
    await page.getByTestId('footer-tab-profile').click()
    await expect(page.getByTestId('profile-username-value')).toContainText(userA)
    await expect(page.getByTestId('friends-list')).toContainText(userB)
